...
import { executorStatusOf, startServer, trustFileOf, type ButlerServer } from './server';
...
async function boot(publicDir?: string): Promise<Ctx> {
  const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'butler2-server-'));
  ...
  // LLM 指到本地不可达地址、单次重试：开发机/CI 环境常带真 BUTLER2_LLM_API_KEY，
  ...
  const LLM_ENV: Record<string, string> = {
    BUTLER2_LLM_BASE_URL: 'http://127.0.0.1:1',
    BUTLER2_LLM_API_KEY: 'test-disabled',
    BUTLER2_LLM_RETRIES: '1',
    BUTLER2_LLM_TIMEOUT_MS: '500',
  };
  const savedEnv = Object.fromEntries(Object.keys(LLM_ENV).map((k) => [k, process.env[k]]));
  Object.assign(process.env, LLM_ENV);
  let server: ButlerServer;
  try {
...
describe('server 集成冒烟（完整装配）', () => {
  test('空数据库启动自动创建唯一系统本机执行机，重启保持幂等', async () => {
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'butler2-local-bootstrap-'));
    cleanups.push(() => fsp.rm(dir, { recursive: true, force: true }));
    const dbPath = path.join(dir, 'butler.db');
    const defaults = {
  ...
  test('healthz → admin 登录 → 建用户/项目/issue → 属主隔离 → 优雅停机', async () => {
    ...
    expect(login.headers.get('set-cookie')).toContain('butler_token=');

    // ---- 建用户：workspace 经 Driver 落到执行机（u<id> 目录 + .butler-keep） ----
    ...
    const keep = await fsp.readFile(path.join(ctx.ws, `u${aliceId}`, '.butler-keep'), 'utf8');
  ...
  test('重启不再重发 admin token（ensureAdminUser 幂等）', async () => {
    ...
    const server2 = await startServer({
      port: 0,
      dbPath: path.join(ctx.dir, 'butler.db'),
...
describe('serveStatic 静态服务（缓存头 + 缺失资源 404）', () => {
  test('/assets immutable、缺失404不回退HTML、index no-cache、无扩展名导航回退', async () => {
    const pub = await fsp.mkdtemp(path.join(os.tmpdir(), 'butler2-pub-'));
