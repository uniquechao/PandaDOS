...
export const BUDGET_ISSUE_TEXT = 800; // issue 正文（v1 2000 按注入预算 2000 联动缩减）
export const BUDGET_GOAL = 250;
export const BUDGET_FEEDBACK = 450; // 卡点 reject 意见 / 测试失败原因
export const BUDGET_TEAM_LIST = 1400; // team 模式子任务清单整体预算
...
export function buildPlanningPrompt(opts: {
  ...
  const boundary = opts.moduleName
    ...
    : '';
  const base = assertBudget(
    boundary +
      `【任务规划】我想做：${issueText(opts.issue, boundary ? 640 : BUDGET_ISSUE_TEXT)}${goal ? `（项目目标：${goal}）` : ''}。` +
...
export function buildSubtaskPrompt(opts: {
  ...
  return assertBudget(
    `【实施 子任务 ${idx + 1}/${subtasks.length}】${sub}。` +
...
export function buildReworkPrompt(opts: {
  ...
  return assertBudget(
    `【返工】${head}：${fb}。请在分支 ${opts.branch} 上修复/调整，改完做必要的自测/跑相关测试验证。` +
...
export function buildTestingPrompt(opts: { issue: PromptIssueLike; branch: string }): string {
  return assertBudget(
    `【测试验证】实施已完成（分支 ${opts.branch}）。请完整跑本项目的测试/编译门（类型检查、单测等），` +
 ...
 */
export function buildReplanRequest(): string {
  return (
 ...
 */
export function buildClarifyContinue(): string {
  return (
...
export function buildNudge(opts: {
  ...
  return `测试都通过了就单独输出一行 STAGE_DONE:${id}:testing（前后别带其它内容）；没通过就继续处理（无法通过就输出一行 TESTS_FAILED:${id} 原因）；${clarifyHint}。`;
}
