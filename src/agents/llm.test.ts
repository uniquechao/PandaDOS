...
describe('配置读取（形状见 llm.ts LlmConfig 注释）', () => {
  const ENV_KEYS = [
    ...
    'PANDA_LLM_MODEL',
    'PANDA_LLM_API_KEY',
    'PANDA_LLM_TEMPERATURE',
    'PANDA_LLM_TIMEOUT_MS',
    'PANDA_LLM_RETRIES',
    'PANDA_LLM_MAX_CONCURRENT',
  ];
  ...
  test('DB 字段优先，只有 NULL 字段才回落环境变量', () => {
    ...
    process.env.PANDA_LLM_BASE_URL = 'https://env.example/v1';
    process.env.PANDA_LLM_MODEL = 'env-model';
    process.env.PANDA_LLM_API_KEY = 'env-key';
