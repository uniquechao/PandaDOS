 ...
 * - 环境变量（DB 未保存对应字段时兜底）：PANDA_LLM_BASE_URL / PANDA_LLM_MODEL /
 *   PANDA_LLM_API_KEY / PANDA_LLM_TEMPERATURE / PANDA_LLM_TIMEOUT_MS /
 *   PANDA_LLM_RETRIES / PANDA_LLM_MAX_CONCURRENT
 */
...
export function loadLlmConfig(db?: Database): LlmConfig {
  ...
  const e = process.env;
  return {
    baseUrl: row.base_url ?? e.PANDA_LLM_BASE_URL ?? DEFAULT_LLM_CONFIG.baseUrl,
    model: row.model ?? e.PANDA_LLM_MODEL ?? DEFAULT_LLM_CONFIG.model,
    apiKey: row.api_key ?? e.PANDA_LLM_API_KEY ?? DEFAULT_LLM_CONFIG.apiKey,
    temperature: row.temperature ?? envNum(e.PANDA_LLM_TEMPERATURE) ?? DEFAULT_LLM_CONFIG.temperature,
    timeoutMs: row.timeout_ms ?? envNum(e.PANDA_LLM_TIMEOUT_MS) ?? DEFAULT_LLM_CONFIG.timeoutMs,
    retries: row.retries ?? envNum(e.PANDA_LLM_RETRIES) ?? DEFAULT_LLM_CONFIG.retries,
    backoffMs: DEFAULT_LLM_CONFIG.backoffMs,
    maxConcurrent:
      row.max_concurrent ?? envNum(e.PANDA_LLM_MAX_CONCURRENT) ?? DEFAULT_LLM_CONFIG.maxConcurrent,
  };
}
