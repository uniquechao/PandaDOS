-- 073_design_run_conversation_scope: every execution conversation binding stays in one project.
-- This replaces the historical design migration 060. Some deployed databases used id 60 for
-- 060_notify_gate_requests.sql, so the design constraint must have its own globally unique id.
-- Rebuilding is intentionally safe when the old design 060 already ran: both the 059 and scoped
-- table shapes expose the same columns copied below.

CREATE UNIQUE INDEX IF NOT EXISTS idx_design_execution_runs_id_project
  ON design_execution_runs(id, project_id);
CREATE UNIQUE INDEX IF NOT EXISTS idx_project_modules_id_project
