...
- 本机与 SSH 执行机
- 项目成员、订阅、同企业飞书登录/绑定、私聊项目问答、通知和审批卡片
- 十种 UI 语言；产品文案按用户语言显示，同时逐字保留 issue、代码、命令、路径、
  终端输出、Git 内容和已有对话

## 飞书接入

在「管理 → 飞书」配置企业自建应用，同事即可使用各自的飞书身份登录，在机器人私聊中
选择有权限的项目、向项目 PM 提问，并接收订阅通知与审批卡片。登录和消息独立开关，
管理页提供凭据验证、连接状态、重连以及发送给当前管理员本人的测试消息。

完整的配置步骤、私聊指令、权限边界、排障和当前限制见[飞书使用指南](docs/feishu.md)。
